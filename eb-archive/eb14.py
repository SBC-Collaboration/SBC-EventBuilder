...
tabControl.add(tab4, text='Playback Images')
...
name_list1 = []
name_list2 = []
img_0 = ttk.Label(tab3)
img_1 = ttk.Label(tab3)
img_2 = ttk.Label(tab3)
name_0 = ttk.Label(tab3)
name_1 = ttk.Label(tab3)
name_2 = ttk.Label(tab3)
...
def image_buttons():
    ...
    name_0.config(text=name_list0[image_number])
    name_1.config(text=name_list1[image_number])
    name_2.config(text=name_list2[image_number])
    img_0.config(image=image_list0[image_number])
    img_1.config(image=image_list1[image_number])
    img_2.config(image=image_list2[image_number])
...
button_forward = ttk.Button(tab3, text='>>', command=forward, state=DISABLED)
...
##############################################
########## Tab 4: Playback Images ############
##############################################
image_player_list = []
image_player_names = []
image_player_number = 0
image_player_name = ttk.Label(tab4)
image_player_image = ttk.Label(tab4)
image_player_name.grid(row=10, column=14, padx=padx, pady=pady)
image_player_image.grid(row=11, column=14, padx=padx, pady=pady)
image_player_directory_label = ttk.Label(tab4, text='CurrDir: ')
image_player_directory_label.grid(row=34, column=10, columnspan=10, padx=padx, pady=pady)
active_state = False
pause_state = False

def load_dir2():
    global image_player_list
    global image_player_names
    global image_player_directory_label
    directory = fd.askdirectory(parent=tk_root, initialdir=curr_directory)
    image_player_list, image_player_names = image_walk(directory, curr_camera.get())
    image_player_directory_label.config(text='CurrDir: '+str(directory))
    button_play.config(state=NORMAL)

button_select_dir2 = ttk.Button(tab4, text='Choose directory', command=load_dir2)
button_select_dir2.grid(row=33, column=14, padx=padx, pady=pady)

ttk.Label(tab4, text='Camera Number: ').grid(row=31, column=12, columnspan=2, padx=padx, pady=pady)
camera_options = ['0', '1', '2']
curr_camera = tk.StringVar(tab4)
camera_select = ttk.OptionMenu(tab4, curr_camera, camera_options[0], *camera_options)
camera_select.grid(row=31, column=14)

ttk.Label(tab4, text='Frequency: ').grid(row=32, column=13, padx=padx, pady=pady)
frequency_spinbox = ttk.Spinbox(master=tab4, width=10, from_=0, to=10000, increment=10, format='%5.0f')
frequency_spinbox.insert(0, 1)
frequency_spinbox.grid(row=32, column=14)

def init_play():
    global image_player_number
    global image_player_name
    global image_player_list
    global image_player_image
    global active_state
    global pause_state
    active_state = True
    if not pause_state:
        image_player_number = 0
        image_player_name = ttk.Label(tab4, text=str(image_player_names[image_player_number]))
        image_player_name.grid(row=10, column=13, columnspan=10, padx=padx, pady=pady)
        image_player_image = ttk.Label(tab4, image=image_player_list[image_player_number])
        image_player_image.grid(row=11, column=13, columnspan=10, padx=padx, pady=pady)
        button_play.config(text='Pause', command=pause)
        image_player_number += 1
        tab4.after(int(frequency_spinbox.get()), play)
    else: 
        button_play.config(text='Pause', command=pause)
        pause_state = False
        active_state = True
...
def play():
    ...
    global active_state
    global pause_state
    if (image_player_number < len(image_player_list) and active_state and not pause_state):
        image_player_name.config(text=str(image_player_names[image_player_number]))
        image_player_image.config(image=image_player_list[image_player_number])
        image_player_number += 1
        tab4.after(int(frequency_spinbox.get()), play)      
    else:
        button_play.config(text='Play', command=init_play)
...
def pause(event=None):
    ...
    active_state = False
    pause_state = True
    button_forward2.config(state=NORMAL)
    button_back2.config(state=NORMAL)


def forward2(event=None):
    global image_player_number
    global button_forward2
    if (image_player_number+2 == len(image_player_list)):
        image_player_number += 1
        scroll()
        button_forward2.config(state=DISABLED)
    elif (image_player_number+1 < len(image_player_list)):
        button_back2.config(state=NORMAL)
        image_player_number += 1
        scroll()
    else:
...
def back2(event=None):
    global image_player_number
    global button_back2
    if image_player_number == 1:
        image_player_number -= 1
        scroll()
        button_back2.config(state=DISABLED)
    elif image_player_number > 0:
        button_forward2.config(state=NORMAL)
        image_player_number -= 1
        scroll()
    else:
...
button_play = ttk.Button(tab4, text='Play', command=init_play, state=DISABLED)
button_play.grid(row=20, column=14, padx=padx, pady=pady)

button_forward2 = ttk.Button(tab4, text='>>', command=forward2, state=DISABLED)
button_back2 = ttk.Button(tab4, text='<<', command=back2, state=DISABLED)
button_back2.grid(row=20, column=13, padx=padx, pady=pady)
button_forward2.grid(row=20, column=15, padx=padx, pady=pady)

tk_root.bind('<Left>', back2)
tk_root.bind('<Right>', forward2)
tk_root.bind('<space>', init_play)
