...
class EB:
    ...
    # --- to inactive. Turn off trig_enable signal. Send a trig_reset signal. Call wait_for_end.
    def cleanup(self):
        for i in range(len(InPins)):
            if not GPIO.input(InPins[i]):
                self.set_status_off(i+1)
...
# Returns a list of all the images (png, jpg, bmp) in a directory resized to height = baseheight pixels
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            if extension == '.png' or '.jpg' or '.bmp':
                # looks at the fourth character in image name to determine camera number
...
def image_buttons():
    ...
    # Adding images onto the tkinter grid and updated buttons
    img_0.grid(row=0, column=0, columnspan=3, padx=padx, pady=pady)
    img_1.grid(row=0, column=3, columnspan=3, padx=padx, pady=pady)
    img_2.grid(row=0, column=6, columnspan=3, padx=padx, pady=pady)
